...
static int r_R1(struct SN_env * z) {
    if (!(z->I[0] <= z->c)) return 0; /* $p1 <= <integer expression>, line 47 */
    return 1;
}
...
static int r_step1(struct SN_env * z) {
    ...
    {   int ret = r_R1(z); /* call R1, line 49 */
        if (ret == 0) return 0;
        if (ret < 0) return ret;
    }
    {   int ret = slice_del(z); /* delete, line 231 */
        if (ret < 0) return ret;
    }
    return 1;
}
...
static int r_step2(struct SN_env * z) {
    while(1) { /* repeat, line 234 */
        int m1 = z->l - z->c; (void)m1;
        {   int mlimit; /* setlimit, line 235 */
            int m2 = z->l - z->c; (void)m2;
            if (z->c < z->I[0]) goto lab0;
            z->c = z->I[0]; /* tomark, line 235 */
            mlimit = z->lb; z->lb = z->c;
            z->c = z->l - m2;
            z->ket = z->c; /* [, line 235 */
            if (!(find_among_b(z, a_1, 62))) { z->lb = mlimit; goto lab0; } /* substring, line 235 */
            z->bra = z->c; /* ], line 235 */
            z->lb = mlimit;
        }
        {   int ret = slice_del(z); /* delete, line 305 */
            if (ret < 0) return ret;
        }
        continue;
    lab0:
...
static int r_fix_conflicts(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 309 */
    if (z->c - 3 <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((2621472 >> (z->p[z->c - 1] & 0x1f)) & 1)) return 0; /* substring, line 309 */
    among_var = find_among_b(z, a_2, 11); /* substring, line 309 */
    if (!(among_var)) return 0;
    z->bra = z->c; /* ], line 309 */
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   int ret = slice_from_s(z, 5, s_0); /* <-, line 311 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 311 */
            break;
        case 2:
            {   int ret = slice_from_s(z, 5, s_1); /* <-, line 313 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 313 */
            break;
        case 3:
            {   int ret = slice_from_s(z, 5, s_2); /* <-, line 316 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 316 */
            break;
        case 4:
            {   int ret = slice_from_s(z, 5, s_3); /* <-, line 318 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 318 */
            break;
        case 5:
            {   int ret = slice_from_s(z, 7, s_4); /* <-, line 321 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 321 */
            break;
        case 6:
            {   int ret = slice_from_s(z, 4, s_5); /* <-, line 324 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 324 */
            break;
        case 7:
            {   int ret = slice_from_s(z, 4, s_6); /* <-, line 326 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 326 */
            break;
        case 8:
            {   int ret = slice_from_s(z, 6, s_7); /* <-, line 329 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 329 */
            break;
        case 9:
            {   int ret = slice_from_s(z, 6, s_8); /* <-, line 330 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 330 */
            break;
        case 10:
            {   int ret = slice_from_s(z, 6, s_9); /* <-, line 333 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 333 */
            break;
        case 11:
            {   int ret = slice_from_s(z, 6, s_10); /* <-, line 335 */
                if (ret < 0) return ret;
            }
...
static int r_fix_chdz(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 340 */
    if (z->c - 1 <= z->lb || (z->p[z->c - 1] != 141 && z->p[z->c - 1] != 190)) return 0; /* substring, line 340 */
    among_var = find_among_b(z, a_3, 2); /* substring, line 340 */
    if (!(among_var)) return 0;
    z->bra = z->c; /* ], line 340 */
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   int ret = slice_from_s(z, 1, s_11); /* <-, line 341 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 341 */
            break;
        case 2:
            {   int ret = slice_from_s(z, 1, s_12); /* <-, line 342 */
                if (ret < 0) return ret;
            }
            z->B[0] = 1; /* set CHANGE, line 342 */
            break;
    }
    return 1;
}
...
static int r_fix_gd(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 347 */
    if (z->c - 1 <= z->lb || z->p[z->c - 1] != 100) return 0; /* substring, line 347 */
    among_var = find_among_b(z, a_4, 1); /* substring, line 347 */
    if (!(among_var)) return 0;
    z->bra = z->c; /* ], line 347 */
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   int ret = slice_from_s(z, 1, s_13); /* <-, line 348 */
                if (ret < 0) return ret;
            }
...
extern int lithuanian_UTF_8_stem(struct SN_env * z) {
    z->I[0] = z->l; /* $p1 = <integer expression>, line 357 */
    z->I[1] = z->l; /* $p2 = <integer expression>, line 358 */
    z->I[2] = SIZE(z->p); /* $s = <integer expression>, line 359 */
    {   int c1 = z->c; /* do, line 361 */
        {   int c_keep = z->c; /* try, line 363 */
            {   int c_test = z->c; /* test, line 363 */
                if (!(eq_s(z, 1, s_14))) { z->c = c_keep; goto lab1; } /* literal, line 363 */
                z->c = c_test;
            }
            if (!(z->I[2] > 6)) { z->c = c_keep; goto lab1; } /* $s > <integer expression>, line 363 */
            {   int ret = skip_utf8(z->p, z->c, 0, z->l, + 1); /* hop, line 363 */
                if (ret < 0) { z->c = c_keep; goto lab1; }
                z->c = ret; /* hop, line 363 */
            }
        lab1:
            ;
        }
        {    /* gopast */ /* grouping v, line 365 */
            int ret = out_grouping_U(z, g_v, 97, 371, 1);
            if (ret < 0) goto lab0;
            z->c += ret;
        }
        {    /* gopast */ /* non v, line 365 */
            int ret = in_grouping_U(z, g_v, 97, 371, 1);
            if (ret < 0) goto lab0;
            z->c += ret;
        }
        z->I[0] = z->c; /* setmark p1, line 365 */
        {    /* gopast */ /* grouping v, line 366 */
            int ret = out_grouping_U(z, g_v, 97, 371, 1);
            if (ret < 0) goto lab0;
            z->c += ret;
        }
        {    /* gopast */ /* non v, line 366 */
            int ret = in_grouping_U(z, g_v, 97, 371, 1);
            if (ret < 0) goto lab0;
            z->c += ret;
        }
        z->I[1] = z->c; /* setmark p2, line 366 */
    lab0:
        z->c = c1;
    }
    z->lb = z->c; z->c = z->l; /* backwards, line 369 */

    {   int m2 = z->l - z->c; (void)m2; /* do, line 370 */
        {   int ret = r_fix_conflicts(z); /* call fix_conflicts, line 370 */
            if (ret == 0) goto lab2;
            if (ret < 0) return ret;
        }
    lab2:
    ...
    {   int m3 = z->l - z->c; (void)m3; /* do, line 371 */
        {   int ret = r_step1(z); /* call step1, line 371 */
            if (ret == 0) goto lab3;
            if (ret < 0) return ret;
        }
    lab3:
    ...
    {   int m4 = z->l - z->c; (void)m4; /* do, line 372 */
        {   int ret = r_fix_chdz(z); /* call fix_chdz, line 372 */
            if (ret == 0) goto lab4;
            if (ret < 0) return ret;
        }
    lab4:
    ...
    {   int m5 = z->l - z->c; (void)m5; /* do, line 373 */
        {   int ret = r_step2(z); /* call step2, line 373 */
            if (ret == 0) goto lab5;
            if (ret < 0) return ret;
        }
    lab5:
    ...
    {   int m6 = z->l - z->c; (void)m6; /* do, line 374 */
        {   int ret = r_fix_chdz(z); /* call fix_chdz, line 374 */
            if (ret == 0) goto lab6;
            if (ret < 0) return ret;
        }
    lab6:
    ...
    {   int m7 = z->l - z->c; (void)m7; /* do, line 375 */
        {   int ret = r_fix_gd(z); /* call fix_gd, line 375 */
            if (ret == 0) goto lab7;
            if (ret < 0) return ret;
        }
    lab7:
